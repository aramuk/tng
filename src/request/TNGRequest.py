...
class TNGRequest():
    ...
    def list_snapshot(self, sim='TNG100-1', snapshot=99, start=0, quantity=100, sort_params=dict(), logfile=None):
    ...
    def get_subhalo_json(self, sim, snapshot, subhalo_id, logfile=None):
        """Gets JSON data for requested subhalo"""
        endpoint = f'{sim}/snapshots/{snapshot}/subhalos/{subhalo_id}'
        return self.get(endpoint, logfile=logfile)


    def get_subhalo_image(self, sim, snapshot, subhalo, params=dict(), logfile=None):
        ...
        endpoint = f'{sim}/snapshots/{snapshot}/subhalos/{subhalo}/vis.png'
